...
public class SafeTextSerializer implements SafeSerializer {
    private Object serializableList;
    private Field listAsField;
    private Class cryptoClass;

    public SafeTextSerializer(Object serializableList, Field listAsField, Class cryptoClass) {
    ...
    public void serialize(String filePath) throws IOException {
        ...
        try {
            ...
            if (csvData.length() != 0) {
                ...
                String encryptedObject = (String) encryptionMethod.invoke(cryptoClass.getDeclaredConstructor().newInstance(), csvData.toString().getBytes());
                bufferedWriter.write(encryptedObject.substring(0, encryptedObject.length() - 1));
            }
    ...
    public Object deserialize(String filePath) throws IOException, ClassNotFoundException, Exception {
        ...
        String allObjectsLine;
        if ((allObjectsLine = bufferedReader.readLine()) != null) {
            Method decryptionMethod = cryptoClass.getDeclaredMethod("decode", String.class);
            byte[] objectBytes = (byte[]) decryptionMethod.invoke(cryptoClass.getDeclaredConstructor().newInstance(), allObjectsLine);
            String[] objects = new String(objectBytes).split("#");

            for (String currentObject : objects) {
                String[] objectFieldsTypedValues = currentObject.split(",");
